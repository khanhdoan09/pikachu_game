...
async function loadLevel() {
    ...
    if (level == 1) {
        ...
        document.querySelector(".matrix").style.backgroundImage = "url('/asset/img/background/back1.jpg')"
        levelAlgorithm = await import('../level/level1_none.js');
    } else if (level == 2) {
        document.querySelector("#level").textContent = "Level 2"
        document.querySelector(".matrix").style.backgroundImage = "url('/asset/img/background/back2.jpg')"
        levelAlgorithm = await import('../level/level2_bottom.js')
    } else if (level == 3) {
        document.querySelector("#level").textContent = "Level 3"
        document.querySelector(".matrix").style.backgroundImage = "url('/asset/img/background/back3.jpg')"
        levelAlgorithm = await import('../level/level3_top.js')
    } else if (level == 4) {
        document.querySelector("#level").textContent = "Level 4"
        document.querySelector(".matrix").style.backgroundImage = "url('/asset/img/background/back4.jpg')"
        levelAlgorithm = await import('../level/level4_left.js')
    } else if (level == 5) {
        document.querySelector("#level").textContent = "Level 5"
        document.querySelector(".matrix").style.backgroundImage = "url('/asset/img/background/back5.jpg')"
        levelAlgorithm = await import('../level/level5_right.js')
    } else {
        document.querySelector("#level").textContent = "Level 6"
        document.querySelector(".matrix").style.backgroundImage = "url('/asset/img/background/back6.jpg')"
